...
su -c "dconf load /org/cinnamon/ < ./cinnamon_desktop_backup" faramos
pkill -HUP -f "cinnamon --replace"

# Language & Locale for the non-root user
su -c "localectl set-locale LANG=en_US.UTF-8" faramos
su -c 'localectl set-x11-keymap cz,us " " , grp:alt_shift_toggle' faramos
